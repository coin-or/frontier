...
3. *“Keep the balanced plan and the safest one as finalists. How much should I trust these?”*
   `explore curate` per pick (each carries a `quality` gate) → `solve solver="highs"` → `explore certify`: the exact-MILP overlay names which heuristic points it dominates — the headline at 300 binary options — plus coverage, the NSGA-never-dominates invariant, and corner sharpening. At this scale expect the audit to catch heuristic slack wholesale — a fresh fast-mode frontier can land dominated end to end, finalists included — so the standard next beat is re-curating each finalist's exact counterpart (`explore curate` on the certified point that beats it), which is what makes the step-6 handoff carry certified plans.
4. *“Which picks survive cost inflation, and what does the delivery crunch cost us?”*
   `solve run_scenarios` → `explore scenario_results`: the frontier re-solved per future — which projects hold their place when every cost reads 12% higher against the same $1,550M, and what the 45–70 execution cap prices out. The baked frontier's portfolios run 45–100 projects with a median of exactly 70, and 19 of its 41 plans exceed the cap — the crunch benches roughly half the base frontier; the live count arrives as `survivors_by_scenario` in the payload.
5. *“What's the tightest lever, and can you guarantee we never blow the risk ceiling, whichever feasible plan we land on?”*
   `explore sensitivity` → `explore audit`: integer selections carry no solver duals, so sensitivity returns the frontier-inferred binding analysis plus `suggested_scenarios`. The audit reasons over **every** feasible plan — and here it returns a counterexample witness, because the model as framed carries no risk cap, so no ceiling can hold yet. That is the teaching beat: a guarantee is something you encode — add the ceiling as a hard `objective_bound` constraint and the same audit then proves `holds` across the whole feasible space (a model change, so take it before the handoff and re-solve/re-certify, or record it as the governance follow-up).
