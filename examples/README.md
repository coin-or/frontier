...
**Solver-exact explainability:** the [investment portfolio](investment_portfolio/#explainability--shadow-prices--near-misses) walks through `solve solver="highs" exact=true` → `explore sensitivity` — shadow prices ("where to invest") and reduced costs ("near-misses") read straight from the optimizer on continuous mean-variance (QP) shapes.
