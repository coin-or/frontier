...
**Why Frontier.** The minimize-risk quadratic puts it on Frontier's exact mean-variance (QP) path – and the contractual floors give the duals a job the spreadsheet can't do: price each contract *including its correlated-risk cost*. On the shipped overlay's balanced anchor the two contracts split cleanly: NA01 earns 15% on merit (its 5% floor sits slack, the contract is free), while CN01 is pinned at its 4% floor with a reduced cost of ~7.5 – the measured price of keeping the legacy commitment.
...
4. *“What are our contracts actually costing us, and which supplier just missed the cut?”*
   `explore sensitivity`: solver-exact duals — at the default reference anchor (the balanced mid-frontier point the tool picks), NA01's 5% floor sits slack (it holds 15% on merit; the contract is free) while CN01 is pinned at its 4% floor with a reduced cost of ~7.5, the per-point price of the legacy commitment with correlated risk included; CN02 is the closest near-miss (~0.45), and the reliability floor prices at ~0.83 risk-units per point. (Values as read at that anchor of the shipped exact overlay — duals are anchor-specific marginal rates that shrink sharply toward the min-risk corner, where CN01's price drops to ~0.9; confirm a big move by re-solving without the floor.)
