...
**Why Frontier.** The minimize-risk quadratic puts it on Frontier's exact mean-variance (QP) path – and the contractual floors give the duals a job the spreadsheet can't do: price each contract *including its correlated-risk cost*. On the shipped overlay's balanced anchor the two contracts split cleanly: NA01 earns 15% on merit (its 5% floor sits slack, the contract is free), while CN01 is pinned at its 4% floor with a reduced cost of ~2 – the measured price of keeping the legacy commitment.
...
4. *“What are our contracts actually costing us, and which supplier just missed the cut?”*
   `explore sensitivity`: solver-exact duals — at the default reference anchor (the balanced mid-frontier point the tool picks), NA01's 5% floor sits slack (it holds 15% on merit; the contract is free) while CN01 is pinned at its 4% floor with a reduced cost of ~2, the per-point price of the legacy commitment with correlated risk included; IN04 is the closest near-miss (~0.05 — nearly free to bring in), the 78-reliability floor sits slack at this anchor (price 0), and the binding requirement there is LeadTime (~0.3). (Values as read at that anchor of the shipped exact overlay — duals are anchor-specific marginal rates: at the min-risk corner CN01's price rises to ~0.9; confirm a big move by re-solving without the floor.)
