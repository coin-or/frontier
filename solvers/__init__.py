...
def exact_solver_fits(problem: "Problem") -> tuple[bool, str]:
    ...
    # The QP shape has exactly one quadratic minimand swept against linear objectives: a
    # second quadratic can be neither the minimand nor an epsilon row, and with no linear
    # objective there is nothing to sweep — decline both rather than mis-solve.
    if len(quad) > 1:
    ...
    if quad and len(problem.objectives) == len(quad):
        return False, (
            "the exact QP sweeps its variance minimand against at least one linear (sum/avg) "
            "objective; this model has only the quadratic. Add a linear objective to trade it "
            "against, or explore with the NSGA heuristic."
        )
