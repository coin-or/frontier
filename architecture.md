...
- **Decision-step pointers.** Every explore/decision action's response carries a `guidance_pointer` (`server.py` `_attach_guidance_pointer` + the `_DECISION_GUIDANCE` map) naming the exact `solution_interpreter` section that governs reading *that* output (e.g. `tradeoffs` → *Presentation Order*, `certify` → *Reading the Certificate*, `sensitivity` → *Exact Sensitivity*, or *Binding Analysis* on the frontier-inferred fallback) plus the `get_skill` re-fetch path. This is the runtime half of the index's "tool responses also name the section to re-read" promise: at each decision the agent is nudged to the right section and re-fetches it if it was compacted away. Centralized as one map + helper so the convention stays uniform; error and navigation actions get no pointer.
