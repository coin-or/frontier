...
- **NVIDIA cuOpt** (`solvers/cuopt_backend.py`, `solver="cuopt"`) — the **GPU exact backend**: a convex min-variance **QP** for proportional mean-variance portfolios and a 0/1 **MILP** for binary selection. The QP has two equivalent builds — the term-by-term high-level `Problem` API (`_solve_qp_cuopt`, GPU-verified) and a scalable matrix `data_model` / CSR build (`_solve_qp_cuopt_matrix`) that drops the O(n²) Python-expression construction for a dense covariance to one vectorised conversion; both pack the same covariance, so they return the same optimum (the `_USE_MATRIX_QP` flag selects, default off pending GPU confirmation). Needs `cuopt-cu12` + a GPU, so it runs in Colab. The epsilon-constraint sweep routinely proposes infeasible objective corners (expected — the engine scores them as dominated; under a cardinality/group cap a floor is often unmeetable on the restricted support); HiGHS returns a clean `Infeasible` status on those, but **cuOpt 26.4.0 aborts the process** (`std::terminate`, from its concurrent barrier+dual-simplex solve hitting a primal-infeasible certificate) — a kernel crash uncatchable in Python — so the cuOpt inner solves **pre-screen feasibility on the CPU** before constructing any GPU model and report an infeasible scalarization as `ok=False`, exactly the signal the engine already expects. The MILP path (`_scalarization._lp_relaxation_feasible`, observed crasher) and both QP builds (`_qp_constraints_feasible`) are guarded symmetrically — each a cheap LP feasibility check via scipy's bundled HiGHS, with no false skips (cross-validated against HiGHS over feasible/infeasible grids).
- **HiGHS** (`solvers/highs_backend.py`, `solver="highs"`) — the **CPU exact backend**: the same engine with HiGHS inner solves. Exact **MILP** for binary selection (every Frontier combinatorial constraint — cardinality, force in/out, dependency, exclusion, group limit, objective bound — is linear-integer, handled exactly; the [`capital_project_selection_120`](examples/capital_project_selection_120) showcase) and convex **QP** for mean-variance portfolios. HiGHS can't solve MIQP, but never has to: under a cardinality/group cap the EA picks *which* assets are eligible and HiGHS solves the continuous QP on that support exactly. The solver is a plain `pip install highspy` (CPU, cross-platform, no special index, no GPU), so the exact path — and the shared NSGA engine with it — runs and is tested on the same machine as the engine ([`tests/test_highs_backend.py`](tests/test_highs_backend.py)).
