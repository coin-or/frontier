...
- **Treat any single LP dual defensively — you can't tell when it's degenerate.** A solver-exact dual is exact for the *basis the solver returned*, but LP optima frequently sit at degenerate corners (several constraints binding at once), where that basis is one of several and the dual is one-sided and non-unique — most precise-looking exactly where it's least reliable. Frontier surfaces no degeneracy flag, so you can't know which case you're in: on the **LP** path, lead with the *ranking* of levers (`where_to_invest` order) and present any single shadow price as a direction and rough magnitude, not a quotable rate. A strictly convex **QP** (mean-variance) has a unique optimum, so its duals are far more stable — quote QP shadow prices with more confidence, still anchored to the reference solution.
