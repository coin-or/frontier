...
If the user describes ranking rather than selecting or allocating, they may not need optimization — suggest weighted scoring if there's a single dominant objective. Weighted scoring has a known blind spot, though: a weighted sum only ever lands on the convex hull of the tradeoff surface, so balanced "knee" options can be unreachable at any weights. If the user pre-commits to weights, note what a frontier view would have shown before they lock in.
...
Constraint *types* and how to classify them live in `frontier://skills/problem_framing` (objective_bound, force_include/exclude, cardinality, exclusion_pair, dependency, group_limit) — don't restate them here. The solve-time judgment that stays: **don't over-constrain — fewer constraints produce a richer frontier and more useful tradeoffs.** Constraints are also how *a-priori* preferences enter the model (hard limits set before solving); *interactive* preferences are the re-solve loop itself (see *Preference Timing* in `frontier://skills/problem_framing`).
...
- **Where to invest → which constraint to renegotiate.** The `where_to_invest` ranking shortlists the binding limits; the rates are in each constraint's own units, so pick the highest-leverage knob by pricing each lever as rate × the increment the user could realistically negotiate. Take that one back to the user: *"the volatility cap is the binding lever — each point of it is worth ~X return here; is 20% firm, or a target?"* — then relax it and re-run. The re-solve, not the dual, prices the actual move.
...
- **Sensitivity analysis** (`explore sensitivity`; or the frontier-inferred `binding_analysis`) is *local and marginal*: how the optimal solution responds to a small change in a **single** input — one objective coefficient or one constraint's right-hand side — holding everything else fixed. It's derivative-like and local: valid only while the same constraints stay active at the optimum. Read it as "at this solution, one more unit of X is worth ~Y," not as a forecast of a different world.
...
## Scope Boundaries
- **Owns:** Solve execution — the validate→run→examine→iterate→certify loop, mode selection, solver selection (default NSGA vs optional exact backend), solve-time constraint judgment (don't over-constrain), infeasibility diagnosis, binding-constraint reading, run comparison.
- **Routes to solution_interpreter:** When results are healthy and ready for user presentation — including scenario-result robustness (*Scenario Results Presentation*) and exact-sensitivity field meanings (*Exact Sensitivity*).
- **Routes back to problem_framing:** When infeasibility or bad results indicate a structural problem (missing constraint, wrong objective, approach mismatch). problem_framing owns constraint *types* and approach choice.
