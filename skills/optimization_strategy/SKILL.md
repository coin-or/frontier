...
- **Where to invest → which constraint to renegotiate.** The `where_to_invest` ranking shortlists the binding limits; the rates are in each constraint's own units, so pick the highest-leverage knob by pricing each lever as rate × the increment the user could realistically negotiate. Take that one back to the user: *"the volatility cap is the binding lever — each point of it is worth ~X return here; is 20% firm, or a target?"* — then relax it and re-run. Duals give direction and a shortlist; the differencing re-solve gives any number you'd act on.
